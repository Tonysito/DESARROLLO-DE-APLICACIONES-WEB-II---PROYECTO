...
public class SecurityConfig {
	...
	SecurityFilterChain authorizationServerSecurityFilterChain(HttpSecurity http)
	    ...
	    http
	        ...
	        // Redirigir a login SOLO si la petición es de tipo text/html
	...
	SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http)
		...
		.formLogin(form -> form
	            .loginPage("/login")   // Aquí le decimos que use tu plantilla
	            .permitAll());
...
OAuth2TokenCustomizer<JwtEncodingContext> jwtCustomizer() {
    return (context) -> {
        ...
        Set<String> normalizedRoles = authentication.getAuthorities().stream()
                .map(GrantedAuthority::getAuthority)
                .map(a -> a.replaceFirst("^ROLE_", "")) 
                .map(String::toLowerCase)               // Pasar a minusculas 
                .collect(Collectors.toSet());

      
        String principalRole = normalizedRoles.stream()
                                    .findFirst() 
                                    .orElse("Undefined"); 

        // Access Token: Incluir el rol (singular)
        if (OAuth2TokenType.ACCESS_TOKEN.equals(context.getTokenType())) {
             context.getClaims().claims(c -> c.put("role", principalRole)); 
        }

        // ID token: Incluir el rol y datos de usuario si se pidio openid
        var scopes = context.getAuthorizedScopes();
        if (scopes != null && scopes.contains(OidcScopes.OPENID)) {
            context.getClaims().claims(claims -> {
                claims.put("role", principalRole); // También en el ID Token

                Object pobj = authentication.getPrincipal();
                // Inyectar datos del usuario con CustomUserDetails
                if (pobj instanceof CustomUserDetails cud && cud.getUsuario() != null) {
                    var usuario = cud.getUsuario();
                    
                    claims.put("id", usuario.getIdUsuario()); 
                    claims.put("sub", String.valueOf(usuario.getIdUsuario())); 
                    claims.put("email", usuario.getEmail());
                    claims.put("roles", normalizedRoles);
                    claims.put("name", usuario.getNombre()); 
                    claims.put("preferred_username", usuario.getEmail()); 
                } else {
                    claims.putIfAbsent("sub", authentication.getName());
                }
            });
        }
    };
